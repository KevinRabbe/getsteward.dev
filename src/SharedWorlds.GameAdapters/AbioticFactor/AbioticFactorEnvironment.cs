...
internal static partial class AbioticFactorEnvironment
{
    ...
    internal static void RequireVanillaInstallation(GameInstallation installation)
    {
        ...
        if (installation.Metadata is null ||
            !installation.Metadata.TryGetValue(
                ...
                out var win64Root) ||
            string.IsNullOrWhiteSpace(win64Root))
        {
            throw new InvalidOperationException(
                "Abiotic Factor's Win64 runtime path is unavailable.");
        }

        var fullRoot = Path.GetFullPath(win64Root);
        var markers = new[]
        {
            Path.Combine(fullRoot, "dwmapi.dll"),
            Path.Combine(fullRoot, "version.dll")
        };
        foreach (var marker in markers)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Abiotic Factor mod-loader marker '{Path.GetFileName(marker)}' is present. Steward's current adapter is vanilla-only.");
            }
        }

        var ue4ssRoot = Path.Combine(fullRoot, "ue4ss");
        if (Directory.Exists(ue4ssRoot))
        {
            if (!AbioticFactorWorldDiscovery.IsRegularDirectory(ue4ssRoot) ||
                Directory.EnumerateFileSystemEntries(ue4ssRoot).Any())
            {
                throw new InvalidOperationException(
                    "Abiotic Factor's UE4SS directory is linked or non-empty. Steward's current adapter is vanilla-only.");
            }
        }
    }
    ...
    internal static string ReadRequiredBuildId(GameInstallation installation)
    {
        ...
        using (stream)
        {
            ...
            var match = SteamBuildIdRegex().Match(
                Encoding.UTF8.GetString(bytes, 0, total));
