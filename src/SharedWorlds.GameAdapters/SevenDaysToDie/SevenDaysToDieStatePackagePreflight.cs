...
internal static class SevenDaysToDieStatePackagePreflight
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (archive.Entries.Count > maxEntries)
        {
            throw new InvalidDataException(
                $"7 Days to Die state package contains more than {maxEntries} archive entries; Steward stopped before extraction.");
        }
        ...
        {
            throw new InvalidDataException(
                "7 Days to Die state package declares an impossible total extraction size.",
                exception);
        }
        ...
        long? availableBytesOverride = null)
    {
        ...
        {
            throw new InvalidDataException(
                "7 Days to Die state package declares an impossible extraction size.",
                exception);
        }
        ...
        if (availableBytes.Value < requiredBytes)
        {
            throw new IOException(
                $"7 Days to Die state package requires {requiredBytes} bytes of free space for safe extraction, but only {availableBytes.Value} bytes are available.");
        }
    }
}
