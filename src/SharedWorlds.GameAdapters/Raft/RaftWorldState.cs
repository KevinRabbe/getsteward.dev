...
internal static class RaftWorldState
{
                ...
                File.Exists(rollbackPath))
            {
                ...
                catch (Exception rollbackException)
                {
                    throw new AggregateException(
                        "Raft state restore failed and the previous prepared World could not be rolled back automatically.",
                        restoreException,
                        rollbackException);
                }
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (source.Length != destination.Length)
        {
            throw new InvalidDataException(
                "Raft World copy did not finish at the expected byte length.");
        }
    }
        ...
        string description)
    {
        ...
        {
            throw new InvalidOperationException(
                $"{description} must be a .rgd file: {path}");
        }
        ...
        {
            throw new InvalidOperationException(
                $"{description} is empty and cannot represent a usable Raft World: {path}");
        }
    }
        ...
        long packageBytes)
    {
        ...
        {
            throw new InvalidDataException(
                "Raft state package declares an impossible file size.",
                ex);
        }
        ...
        try
        {
            ...
            if (drive.IsReady && drive.AvailableFreeSpace < required)
            {
                throw new IOException(
                    $"Raft state restore requires {required} bytes of free space, but only {drive.AvailableFreeSpace} bytes are available.");
            }
        }
