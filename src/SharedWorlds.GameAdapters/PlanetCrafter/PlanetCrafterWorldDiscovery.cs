...
internal static class PlanetCrafterWorldDiscovery
{
    internal const string NativeBackupFileName = "Backup.json";
    ...
    public static IReadOnlyList<DetectedWorld> Discover(GameInstallation installation)
    {
        ...
        if (installation.Metadata is null ||
            !installation.Metadata.TryGetValue(
                PlanetCrafterInstallationDiscovery.WorldRootPathKey,
                out var worldRoot) ||
    ...
    internal static IReadOnlyList<DetectedWorld> DiscoverFromWorldRoot(string worldRoot)
    {
        ...
        {
            return Directory
                .EnumerateFiles(fullRoot, "*.json", SearchOption.TopDirectoryOnly)
                .Where(path =>
                    !string.Equals(
