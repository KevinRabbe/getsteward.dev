...
internal static class IcarusWorldState
{
        ...
        EnvironmentManifest requiredEnvironment)
    {
        ...
        return new PreparedWorld(
            installation,
            IcarusWorkspaceOwnership.Create(),
            requiredEnvironment,
            DisplayName: null);
    }
        ...
        try
        {
            ...
            if (File.Exists(destinationPath))
            {
                RequireRegularWorldFile(
                    destinationPath,
                    "existing prepared ICARUS World");
                ...
                File.Exists(rollbackPath))
            {
                ...
                catch (Exception rollbackException)
                {
                    throw new AggregateException(
                        "ICARUS state restore failed and the previous prepared World could not be rolled back automatically.",
                        restoreException,
                        rollbackException);
                }
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (source.Length != destination.Length)
        {
            throw new InvalidDataException(
                "ICARUS World copy did not finish at the expected byte length.");
        }
    }
        ...
        string description)
    {
        ...
        {
            throw new InvalidOperationException(
                $"{description} is empty and cannot represent a usable ICARUS World: {path}");
        }
    }
        ...
        long packageBytes)
    {
        ...
        {
            throw new InvalidDataException(
                "ICARUS state package declares an impossible file size.",
                ex);
        }
        ...
        try
        {
            ...
            if (drive.IsReady && drive.AvailableFreeSpace < required)
            {
                throw new IOException(
                    $"ICARUS state restore requires {required} bytes of free space, but only {drive.AvailableFreeSpace} bytes are available.");
            }
        }
