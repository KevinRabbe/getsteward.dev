...
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), $"sharedworlds-raft-{Guid.NewGuid():N}");
    ...
    public void InstallationDiscoveryFindsSteamRaftAndOwnedRoots()
    {
        ...
        File.WriteAllText(manifest, "\"AppState\" { \"appid\" \"648800\" \"buildid\" \"12345\" }");
        var userRoot = Path.Combine(_root, "users");
        var rmlRoot = Path.Combine(_root, "rml");

        var installation = Assert.Single(RaftInstallationDiscovery.DiscoverFromSteamLibraries([library], userRoot, rmlRoot));

        Assert.Equal(Path.GetFullPath(installRoot), installation.RootPath);
        Assert.Equal("steam", installation.Source);
        Assert.Equal(Path.GetFullPath(Path.Combine(installRoot, "Raft.exe")), installation.Metadata![RaftInstallationDiscovery.ClientExecutablePathKey]);
        Assert.Equal(Path.GetFullPath(userRoot), installation.Metadata[RaftInstallationDiscovery.UserRootPathKey]);
        Assert.Equal(Path.GetFullPath(Path.Combine(installRoot, "mods")), installation.Metadata[RaftInstallationDiscovery.ModsRootPathKey]);
        Assert.Equal(Path.GetFullPath(rmlRoot), installation.Metadata[RaftInstallationDiscovery.RmlRootPathKey]);
    }
    ...
    public void WorldDiscoveryFindsOnlySameNameCurrentWorldFile()
    {
        ...
        File.WriteAllBytes(Path.Combine(profile, "Player", "RGD_Users.rgd"), [5]);
        var invalid = Path.Combine(userRoot, "User_not-a-steamid", "World", "Wrong");
        ...
        var detected = Assert.Single(RaftWorldDiscovery.DiscoverFromUserRoot(userRoot));

        Assert.Equal("OceanHome", detected.DisplayName);
        Assert.Equal("local:User_76561198000000000:OceanHome", detected.Id);
        Assert.Equal(Path.GetFullPath(Path.Combine(world, "OceanHome.rgd")), detected.SourcePath);
    }
    ...
    public void WorldDiscoveryRejectsLinkedCurrentWorldFile()
    {
        ...
        var world = Path.Combine(userRoot, "User_76561198000000000", "World", "Linked");
        Directory.CreateDirectory(world);
        var outside = Path.Combine(_root, "outside.rgd");
        File.WriteAllBytes(outside, [1, 2, 3]);
        var link = Path.Combine(world, "Linked.rgd");
        File.CreateSymbolicLink(link, outside);
        try { Assert.Empty(RaftWorldDiscovery.DiscoverFromUserRoot(userRoot)); }
    ...
    public void EnvironmentInspectionRejectsGameRootMods()
    {
        ...
        var exception = Assert.Throws<InvalidOperationException>(() => RaftEnvironment.Inspect(installation));
    ...
    public async Task CaptureRestoreCapturePreservesOnlyCurrentWorldBytes()
    {
        ...
        var bytes = Enumerable.Range(0, 16384).Select(index => (byte)((index * 13) % 251)).ToArray();
        File.WriteAllBytes(source, bytes);
        File.WriteAllBytes(Path.Combine(worldDir, "backup.rgd"), [9, 9, 9]);
        Directory.CreateDirectory(Path.Combine(profile, "Player"));
        File.WriteAllBytes(Path.Combine(profile, "Player", "RGD_Users.rgd"), [8, 8, 8]);
        var detected = Assert.Single(RaftWorldDiscovery.DiscoverFromUserRoot(userRoot));
        ...
        try
        {
            ...
            Assert.Equal(bytes, File.ReadAllBytes(Path.Combine(workspace, "save", "world.rgd")));
        ...
        {
            if (Directory.Exists(workspace)) await adapter.FinalizePreparedWorldAsync(prepared, PreparedWorldDisposition.Discard);
        }
    }
    ...
    public async Task RestoreRejectsEmptyWorldPackage()
    {
        ...
        var adapter = new RaftAdapter();
        var prepared = await adapter.PrepareEnvironmentAsync(installation, environment);
        try { await Assert.ThrowsAsync<InvalidOperationException>(() => adapter.RestoreStateAsync(prepared, new StatePackage("empty", empty))); }
        finally { await adapter.FinalizePreparedWorldAsync(prepared, PreparedWorldDisposition.Discard); }
    }
    ...
    public async Task PrepareEnvironmentRejectsDifferentSteamBuild()
    {
        ...
        var required = new EnvironmentManifest(1, "raft", "22222", [], new Dictionary<string, string>(StringComparer.Ordinal));
        var adapter = new RaftAdapter();
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => adapter.PrepareEnvironmentAsync(installation, required));
    ...
    {
        Assert.True(RaftWorldDiscovery.TryGetSteamId("User_76561198000000000", out var steamId));
        Assert.Equal(76561198000000000UL, steamId);
        Assert.False(RaftWorldDiscovery.TryGetSteamId("User_076561198000000000", out _));
        Assert.False(RaftWorldDiscovery.TryGetSteamId("76561198000000000", out _));
    }

    [Fact]
    public void AdapterAdvertisesOnlyExactGameVersion()
    {
        Assert.Equal(GameAdapterCapabilities.ExactGameVersion, new RaftAdapter().Capabilities);
    }
    ...
    private GameInstallation CreateInstallation(string buildId)
    {
        ...
        var manifest = Path.Combine(library, "steamapps", "appmanifest_648800.acf");
        Directory.CreateDirectory(Path.GetDirectoryName(manifest)!);
        File.WriteAllText(manifest, $"\"AppState\" {{ \"appid\" \"648800\" \"buildid\" \"{buildId}\" }}");
        var userRoot = Path.Combine(_root, $"user-root-{Guid.NewGuid():N}");
        var rmlRoot = Path.Combine(_root, $"rml-{Guid.NewGuid():N}");
        return new GameInstallation(
            $"raft:test:{Guid.NewGuid():N}", installRoot, "test",
            new Dictionary<string, string>(StringComparer.Ordinal)
            {
                [RaftInstallationDiscovery.ClientExecutablePathKey] = Path.Combine(installRoot, "Raft.exe"),
                [RaftInstallationDiscovery.SteamManifestPathKey] = manifest,
                [RaftInstallationDiscovery.UserRootPathKey] = userRoot,
                [RaftInstallationDiscovery.ModsRootPathKey] = Path.Combine(installRoot, "mods"),
                [RaftInstallationDiscovery.RmlRootPathKey] = rmlRoot,
                [RaftInstallationDiscovery.GameSteamAppIdKey] = RaftInstallationDiscovery.GameSteamAppId
            });
    }

    public void Dispose()
    {
        foreach (var package in _packages) TryDeleteFile(package);
