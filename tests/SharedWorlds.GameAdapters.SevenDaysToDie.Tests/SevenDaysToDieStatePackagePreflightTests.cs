...
public sealed class SevenDaysToDieStatePackagePreflightTests : IDisposable
{
    private readonly string _root = Path.Combine(
        Path.GetTempPath(),
        $"sharedworlds-7dtd-preflight-{Guid.NewGuid():N}");
    ...
    public void DeclaredExtractionBytesSumOnlyFileEntries()
    {
        ...
        var bytes = SevenDaysToDieStatePackagePreflight.GetDeclaredExtractionBytes(
            archive,
            maxEntries: 10);
    ...
    public void FreeSpaceReserveFailsClosedWhenDeclaredExtractionCannotFit()
    {
        var exception = Assert.Throws<IOException>(() =>
            SevenDaysToDieStatePackagePreflight.EnsureSufficientFreeSpace(
