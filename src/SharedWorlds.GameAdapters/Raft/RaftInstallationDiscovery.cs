...
internal static partial class RaftInstallationDiscovery
{
    ...
    public static IReadOnlyList<GameInstallation> Discover()
    {
        ...
        if (string.IsNullOrWhiteSpace(profile) || string.IsNullOrWhiteSpace(roaming)) return [];
        var userRoot = Path.Combine(profile, "AppData", "LocalLow", "Redbeet Interactive", "Raft", "User");
        ...
        string rmlRoot)
    {
        ...
        foreach (var raw in steamLibraries)
        {
            ...
            var root = NormalizePathOrNull(Path.Combine(library, "steamapps", "common", "Raft"));
            if (root is null || !seen.Add(root)) continue;
            ...
            var manifest = Path.Combine(library, "steamapps", $"appmanifest_{GameSteamAppId}.acf");
            if (File.Exists(manifest)) metadata[SteamManifestPathKey] = Path.GetFullPath(manifest);
            installations.Add(new GameInstallation($"raft:{root}", root, "steam", metadata));
        }
    ...
    private static IEnumerable<string> DiscoverSteamLibraries()
    {
        ...
        try
        {
            ...
            if (key?.GetValue("SteamPath") is string steamPath && !string.IsNullOrWhiteSpace(steamPath)) roots.Add(steamPath);
        }
        catch { }
        ...
        foreach (var steamRoot in roots)
        {
            ...
            try
            {
                foreach (Match match in SteamLibraryPathRegex().Matches(File.ReadAllText(vdf)))
                {
                    ...
                    if (normalized is not null) libraries.Add(normalized);
                }
            }
