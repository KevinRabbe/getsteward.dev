...
internal static class ConanExilesEnhancedWorldState
{
    private const string PreparedSaveDirectoryName = "save";
    private const string PreparedWorldFileName = "world.db";
        ...
        EnvironmentManifest requiredEnvironment)
    {
        ...
        return new PreparedWorld(
            installation,
            ConanExilesEnhancedWorkspaceOwnership.Create(),
            requiredEnvironment,
            DisplayName: null);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                RequireRegularDatabase(
                    destinationPath,
                    "existing prepared Conan Exiles Enhanced World");
                ...
                File.Exists(rollbackPath))
            {
                ...
                catch (Exception rollbackException)
                {
                    throw new AggregateException(
                        "Conan Exiles Enhanced state restore failed and the previous prepared World could not be rolled back automatically.",
                        restoreException,
                        rollbackException);
                }
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (source.Length != destination.Length)
        {
            throw new InvalidDataException(
                "Conan Exiles Enhanced database copy did not finish at the expected byte length.");
        }
    }
    ...
    private static void RequireRegularDatabase(
        string path,
        string description)
    {
        if (!string.Equals(
        ...
        {
            throw new InvalidOperationException(
                $"{description} must be a .db file: {path}");
        }
        ...
        {
            throw new InvalidOperationException(
                $"{description} is empty and cannot represent a usable Conan Exiles Enhanced World: {path}");
        }
    }
        ...
        long packageBytes)
    {
        ...
        {
            throw new InvalidDataException(
                "Conan Exiles Enhanced state package declares an impossible file size.",
                exception);
        }
        ...
        try
        {
            ...
            if (drive.IsReady && drive.AvailableFreeSpace < required)
            {
                throw new IOException(
                    $"Conan Exiles Enhanced state restore requires {required} bytes of free space, but only {drive.AvailableFreeSpace} bytes are available.");
            }
        }
