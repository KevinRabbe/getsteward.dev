...
public sealed class PalworldStatePackagePreflightTests : IDisposable
{
    private readonly string _root = Path.Combine(
        Path.GetTempPath(),
        $"sharedworlds-palworld-preflight-{Guid.NewGuid():N}");
    ...
    public void DeclaredExtractionBytesSumOnlyFileEntries()
    {
        ...
        var bytes = PalworldStatePackagePreflight.GetDeclaredExtractionBytes(
            archive,
            maxEntries: 10);
    ...
    public void EntryCountLimitStopsBeforeExtraction()
    {
        ...
        var exception = Assert.Throws<InvalidDataException>(() =>
            PalworldStatePackagePreflight.GetDeclaredExtractionBytes(
                archive,
                maxEntries: 2));
    ...
    public void FreeSpaceReserveFailsClosedWhenDeclaredExtractionCannotFit()
    {
        var exception = Assert.Throws<IOException>(() =>
            PalworldStatePackagePreflight.EnsureSufficientFreeSpace(
