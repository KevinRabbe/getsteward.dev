...
Require-SafeAbsoluteLinuxPath $HostEnvironmentPath 'HostEnvironmentPath'
...
$quotedImageId = ConvertTo-ShellSingleQuoted $backendImageId
$quotedImageTag = ConvertTo-ShellSingleQuoted $backendImageTag
$quotedTarRelative = ConvertTo-ShellSingleQuoted $backendTarRelativePath
$quotedTarHash = ConvertTo-ShellSingleQuoted $backendTarSha256.ToLowerInvariant()
$quotedEnvironmentPath = ConvertTo-ShellSingleQuoted $HostEnvironmentPath
$deployText = @"
#!/usr/bin/env bash
set -euo pipefail

bundle_dir="`${1:?usage: deploy-exact-candidate.sh <bundle-directory>}"
env_file=${quotedEnvironmentPath}
image_id=${quotedImageId}
image_tag=${quotedImageTag}
tar_relative=${quotedTarRelative}
tar_sha256=${quotedTarHash}
...
$quotedApiBaseUrl = ConvertTo-ShellSingleQuoted $apiBaseUrl
$verifyText = @"
#!/usr/bin/env bash
set -euo pipefail

api_base_url=${quotedApiBaseUrl}
