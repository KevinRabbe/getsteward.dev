...
internal static class SatisfactoryWorldState
{
    private const string PreparedSaveDirectoryName = "save";
    private const string PreparedWorldFileName = "world.sav";
        ...
        EnvironmentManifest requiredEnvironment)
    {
        ...
        return new PreparedWorld(
            installation,
            SatisfactoryWorkspaceOwnership.Create(),
            requiredEnvironment,
            DisplayName: null);
    }
        ...
        try
        {
            ...
            if (File.Exists(destinationPath))
            {
                RequireRegularWorldFile(
                    destinationPath,
                    "existing prepared Satisfactory World");
                ...
                File.Exists(rollbackPath))
            {
                ...
                catch (Exception rollbackException)
                {
                    throw new AggregateException(
                        "Satisfactory state restore failed and the previous prepared World could not be rolled back automatically.",
                        restoreException,
                        rollbackException);
                }
            }
        ...
        string description)
    {
        ...
        {
            throw new InvalidOperationException(
                $"{description} must be a .sav file: {path}");
        }
        ...
        {
            throw new InvalidOperationException(
                $"{description} is empty and cannot represent a usable Satisfactory World: {path}");
        }
    }
        ...
        long packageBytes)
    {
        ...
        catch (OverflowException exception)
        {
            throw new InvalidDataException(
                "Satisfactory state package declares an impossible file size.",
                exception);
        }
        ...
        try
        {
            ...
            if (drive.IsReady && drive.AvailableFreeSpace < required)
            {
                throw new IOException(
                    $"Satisfactory state restore requires {required} bytes of free space, but only {drive.AvailableFreeSpace} bytes are available.");
            }
        }
