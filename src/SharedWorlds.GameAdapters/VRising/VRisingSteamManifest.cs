...
internal static partial class VRisingSteamManifest
{
    internal const long MaximumBytes = 4L * 1024 * 1024;

    internal static VRisingSteamManifestRecord ReadRequired(string manifestPath)
    {
        ...
        if (total > maximum)
        {
            throw new InvalidOperationException(
                $"V Rising's Steam manifest exceeded Steward's {MaximumBytes}-byte metadata safety limit while being read: {fullPath}");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Steam buildid was not found in V Rising's manifest: {fullPath}");
        }

        if (!installDirMatch.Success || string.IsNullOrWhiteSpace(installDirMatch.Groups[1].Value))
        {
            throw new InvalidOperationException(
                $"Steam installdir was not found in V Rising's manifest: {fullPath}");
        }
        ...
        {
            throw new InvalidOperationException(
                $"V Rising's Steam installdir is not a safe library-relative directory name: {fullPath}");
        }

        return new VRisingSteamManifestRecord(
    ...
    private static FileStream OpenRegularBounded(string fullPath)
    {
        ...
        {
            throw new InvalidOperationException(
                $"V Rising's Steam manifest could not be opened safely: {fullPath}",
                exception);
        }
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    $"V Rising's Steam manifest is not a regular owned file: {fullPath}");
            }

            if (stream.Length > MaximumBytes)
            {
                throw new InvalidOperationException(
                    $"V Rising's Steam manifest exceeds Steward's {MaximumBytes}-byte metadata safety limit: {fullPath}");
            }
...
internal sealed record VRisingSteamManifestRecord(
    string BuildId,
