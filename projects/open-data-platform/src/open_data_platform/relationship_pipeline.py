...
from .relationship_parser import parse_relationship_snapshot
from .relationship_product import build_relationship_product
from .relationship_release import build_relationship_release
...
def run_relationship_pipeline(
    *,
    ...
    with pipeline_lock(data_root) as lock_metadata:
        ...
        append_event(event_log, event="RELATIONSHIP_PIPELINE_STARTED", payload={"run_id": run_id})
        try:
            ingest = _timed(
                stage_seconds,
                "ingest",
            ...
            parsed = _timed(
                stage_seconds,
                "parse",
                lambda: parse_relationship_snapshot(
                    data_root,
                    snapshot_id,
            ...
            product = _timed(
                stage_seconds,
                "product",
                lambda: build_relationship_product(
                    data_root,
                    snapshot_id,
            ...
            release = _timed(
                stage_seconds,
                "release",
                lambda: build_relationship_release(
                    data_root,
                    snapshot_id,
                    normalized_root=normalized_root,
            ...
            append_event(
                event_log,
                event="RELATIONSHIP_PIPELINE_FAILED",
