...
A shared account/profile namespace also does not make every object part of one revision. Raft keeps current World state and player-owned inventory/persona state under the same `User_<SteamID64>` profile. Steward discovers `World/<name>/<name>.rgd` as World state and deliberately does not traverse the separate `Player` tree.
