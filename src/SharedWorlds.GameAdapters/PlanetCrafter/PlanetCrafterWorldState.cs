...
internal static class PlanetCrafterWorldState
{
    ...
    {
        ArgumentNullException.ThrowIfNull(world);
        PlanetCrafterWorkspaceOwnership.RequireOwned(world.WorkingDirectory);
        return CaptureFileAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            PlanetCrafterWorkspaceOwnership.RequireOwnedTree(world.WorkingDirectory);
...
internal static class PlanetCrafterWorkspaceOwnership
{
    ...
    private static InvalidOperationException Refuse(string path)
        => new(
            $"Refusing to use unrecognized or linked Planet Crafter Steward workspace '{path}'.");
}
