...
internal static class PalworldStatePackagePreflight
{
        ...
        int maxEntries)
    {
        ...
        if (archive.Entries.Count > maxEntries)
        {
            throw new InvalidDataException(
                $"Palworld state package contains more than {maxEntries} archive entries; Steward stopped before extraction.");
        }
        ...
        {
            throw new InvalidDataException(
                "Palworld state package declares an impossible total extraction size.",
                exception);
        }
        ...
        long? availableBytesOverride = null)
    {
        ...
        {
            throw new InvalidDataException(
                "Palworld state package declares an impossible extraction size.",
                exception);
        }
        ...
        if (availableBytes.Value < requiredBytes)
        {
            throw new IOException(
                $"Palworld state package requires {requiredBytes} bytes of free space for safe extraction, but only {availableBytes.Value} bytes are available.");
        }
    }
}
